...
export default (props: DpsProps) => {
  ...
  createComputed(() => {
    ...
    const dpsValues = Object.entries(counterState.entities).reduce(
      (acc, entry) => {
        ...
        {(item) => {
          ...
          return (
            <div class={classes.entry} style={{ '--val': bgWidth }}>
              <div class={classes.entryName}>{item.name}</div> <div class={classes.entryValue}>{numberIntl.format(item.dps)}</div>
            </div>
          );
        }}
      </For>
